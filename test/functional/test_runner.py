...
def main():
    ...
    tmpdir = "%s/antomcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and antomcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "antomcoind"]) is not None:
            print("%sWARNING!%s There is already a antomcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "ANTOMCOIND" not in os.environ:
        os.environ["ANTOMCOIND"] = build_dir + '/src/antomcoind' + exeext
        os.environ["ANTOMCOINCLI"] = build_dir + '/src/antomcoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `antomcoin-cli help` (`rpc_interface.txt`).
